...
func (q *Queries) CountPostTagsByTagID(ctx context.Context, tagID uuid.UUID) (int64, error) {
	row := q.db.QueryRowContext(ctx, countPostTagsByTagID, tagID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
func (q *Queries) CreatePostTag(ctx context.Context, arg CreatePostTagParams) (*PostTag, error) {
	row := q.db.QueryRowContext(ctx, createPostTag, arg.TagID, arg.PostID)
	var i PostTag
	err := row.Scan(
		&i.ID,
		&i.TagID,
...
func (q *Queries) DeletePostTagsByPostID(ctx context.Context, arg DeletePostTagsByPostIDParams) ([]*PostTag, error) {
	rows, err := q.db.QueryContext(ctx, deletePostTagsByPostID, arg.DeletedAt, arg.PostID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) GetTagsOrderByCountTags(ctx context.Context, limit int32) ([]*GetTagsOrderByCountTagsRow, error) {
	rows, err := q.db.QueryContext(ctx, getTagsOrderByCountTags, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
