...
func (q *Queries) GetTagByIDs(ctx context.Context, ids []pgtype.UUID) ([]*Tag, error) {
	rows, err := q.db.Query(ctx, getTagByIDs, ids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) GetTagsByTagNameAndPageInfo(ctx context.Context, arg GetTagsByTagNameAndPageInfoParams) ([]*GetTagsByTagNameAndPageInfoRow, error) {
	rows, err := q.db.Query(ctx, getTagsByTagNameAndPageInfo, arg.Limit, arg.Name)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
